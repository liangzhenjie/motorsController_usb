...
void Communication::addCommunication(const QString &portName, const quint32 nBaudRate)
{
    QRegExp rx("\\d+");
    int pos = rx.indexIn(portName);
    if(pos > -1)
    {
        quint8 nPortNum = rx.cap(0).toUInt();
        CommunicateUnit * pUnit = new CommunicateUnit(nPortNum,portName,nBaudRate);
    ...
    {
        qDebug() << "portName error! " << portName;
    }
}
